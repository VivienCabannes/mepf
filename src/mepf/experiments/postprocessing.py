...
def get_validity(res_dir, grid, tol=0, verbose=False):
    ...
    validity = pd.DataFrame(is_valid, columns=['method', 'problem', 'm', 'delta', 'constants', 'valid'])
    ...
    if verbose:
        ...
        if valid_list:
            setup = valid_list[0]
            ind = (validity['problem'] == setup[0]) & (validity['m'] == setup[1]) & (validity['delta'] == setup[2])
    ...
    def stat_report(res, method, problem, m, delta):
        ...
        for constant in grid['constant']:
            ...
            if np.isinf(best.values).any():
                ...
                best_mean = best.mean()
                best_std = best.std()
            yield [method, problem, constant, m, delta, delta_effective, best_mean, best_std, ratio_exp]

    out = process_results(res_dir, grid, stat_report)
    out = pd.DataFrame(out, columns=['method', 'problem', 'constant', 'm', 'delta', 'delta_effective', 'best_mean', 'best_std', 'ratio_exp'])
    return out
...
if __name__ == '__main__':
    ...
    grid = {
        ...
        "problem": ["two", "geometric"],
        "num_classes": [100, 300, 1000],
